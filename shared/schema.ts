...
export const users = pgTable("users", {
  ...
  // Pedido do professor (2026-09): flag independente de "role" — em vez de
  // trocar o role de uma conta existente (o que quebraria o acesso normal
  // dela, ex.: um professor perderia as rotas gated por role==="professor"),
  // "isAdmin" pode ser ligado em cima de qualquer role, inclusive a mesma
  // conta que já é professor. Ver server/ensureSuperAdminAccount.ts.
...
export const marketEvents = pgTable("market_events", {
  ...
  // Pedido do professor (2026-09): antes o sinal do impacto (positivo/negativo)
  // era inferido rigidamente a partir de "type" dentro de calculateEventImpact
  // (server/calculator.ts), o que deixava eventos "regulatorio" sem nenhum
  // efeito numérico (bug). Agora cada evento carrega sua própria polaridade,
  // podendo ser escolhida manualmente pelo professor ou gerada pela IA
  // (ver server/services/aiEventGenerator.ts) — a geração automática pela IA
  // fica pausada até o professor liberar o uso de créditos da API.
  sentiment: text("sentiment").notNull().default("neutro"), // "positivo" | "negativo" | "neutro"
...
export const insertMarketEventSchema = createInsertSchema(marketEvents).omit({
  ...
  sentiment: z.enum(["positivo", "negativo", "neutro"]).default("neutro"),
});
